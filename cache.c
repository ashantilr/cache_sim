...
char *make_block(int block_size) {
  ...
  char* accessed = (char*)malloc(block_size*sizeof(char));
  return accessed;
}
...
Line *make_lines(int line_count, int block_size) {
  ...
  struct Line* lines = (struct Line*)malloc(line_count * sizeof(Line));
...
Set *make_sets(int set_count, int line_count, int block_size) {
  ...
  //
  struct Set* sets = (struct Set*)malloc(set_count*sizeof(Set));
...
Cache *make_cache(int set_bits, int line_count, int block_bits) {
  Cache *cache = (struct Cache*)malloc(sizeof(Cache));
  // TODO:
